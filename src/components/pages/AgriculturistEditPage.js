...
import { MDBContainer, MDBRow, MDBCol, MDBInput, MDBBtn, MDBNotification} from 'mdbreact';
...
class AgriculturistEditPage extends Component {
  ...
  submitHandler = async event => {
    ...
    try {
      var inputVegetation = document.querySelector('#ranch_total_vegetation_area')
      var inputArable = document.querySelector('#ranch_total_arable_area')
      var inputTotal = document.querySelector('#ranch_total_area')
      
      if(this.state.valided) {
        this.setState({'valided': false})
      }
      
      if (!event.target.className.includes("was-validated"))
      event.target.className += " was-validated";
      
      if (parseInt(this.state.ranch_total_arable_area) + parseInt(this.state.ranch_total_vegetation_area) > this.state.ranch_total_area) {
        ...
        const response = await api.put(`/agriculturists/${this.state.id}`, this.state)
      }
    } catch (err) {
  ...
  render() {
    console.log(this.state.valided)
    return (
      <React.Fragment>
        {this.state.valided &&
        <MDBNotification
